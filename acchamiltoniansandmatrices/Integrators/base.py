...
class Integrator:
    ...
    def _assert_parameters(hamiltonian, q_symbols, p_symbols, parameter_subs):
        free_symbols = (
            hamiltonian.free_symbols - set(q_symbols + p_symbols) - set(parameter_subs.keys())
        )
        if len(free_symbols) > 0:
            raise ValueError(
                "Not all the parameters have values. Please assign the value of {}".format(
                    str(list(free_symbols))
                )
            )
    ...
    def _equations_of_motion(self):
        ...
        dp_dt_expr = sp.Matrix(
            [-sp.diff(self.hamiltonian, qi).simplify() for qi in self.q_symbols]
        )
        ...
        def dq_dt(q, p):
            return self._apply_func(dq_dt_func, q, p)

        def dp_dt(q, p):
            return self._apply_func(dp_dt_func, q, p)
    ...
    def plot(self, indices=None):
        ...
        for i in _indices:
            plt.plot(
                self.q_time[i, ...],
                self.p_time[i, ...],
                label=r"$({},{})$".format(self.q_symbols[i], self.p_symbols[i]),
            )
