...
class PositionVerletIntegrator(Integrator):
    def single_step_func(self, q_curr, p_curr, dt):
        q_mid = q_curr + 1.0 / 2 * p_curr * dt
        p_next = p_curr + self.dp_dt(q_mid, p_curr) * dt
        q_next = q_mid + 1.0 / 2 * p_next * dt
        return q_next, p_next


class VelocityVerletIntegrator(Integrator):
    def single_step_func(self, q_curr, p_curr, dt):
        p_mid = p_curr + 1.0 / 2 * self.dp_dt(q_curr, p_curr) * dt
        q_next = q_curr + p_mid * dt
        p_next = p_mid + 1.0 / 2 * self.dp_dt(q_next, p_curr) * dt
        return q_next, p_next
...
class SymplecticIntegrator(Integrator):
    nth_order_coefficients = {
        1: ([1], [1]),
        2: ([0, 1], [0.5, 0.5]),
        3: ([1, -2.0 / 3, 2.0 / 3], [-1.0 / 24, 3.0 / 4, 7.0 / 24]),
        4: (
            [
                1.0 / (2 * (2 - 2 ** (1.0 / 3))),
    ...
    def _create_single_step_func(self, order):
        ...
        def single_step_func(q_curr, p_curr, dt):
            ...
            for c, d in zip(c_list, d_list):
                ...
                print(q_curr)
                print(p_curr)
                print(self.dq_dt(q_curr, p_curr))
                print(self.dp_dt(q_curr, p_curr))
...
class MidpointIntegrator(Integrator):
    def single_step_func(self, q_curr, p_curr, dt):
        q_mid = q_curr + 1.0 / 2 * self.dq_dt(q_curr, p_curr) * dt
        p_mid = p_curr + 1.0 / 2 * self.dp_dt(q_curr, p_curr) * dt
...
class RungeKutta4Integrator(Integrator):
    ...
    def single_step_func(self, q_curr, p_curr, dt):
        ...
        k2_q = self.dq_dt(q_curr + 1.0 / 2 * k1_q * dt, p_curr + 1.0 / 2 * k1_p * dt)
        k2_p = self.dp_dt(q_curr + 1.0 / 2 * k1_q * dt, p_curr + 1.0 / 2 * k1_p * dt)

        k3_q = self.dq_dt(q_curr + 1.0 / 2 * k2_q * dt, p_curr + 1.0 / 2 * k2_p * dt)
        k3_p = self.dp_dt(q_curr + 1.0 / 2 * k2_q * dt, p_curr + 1.0 / 2 * k2_p * dt)

        k4_q = self.dq_dt(q_curr + k3_q * dt, p_curr + k3_p * dt)
        k4_p = self.dp_dt(q_curr + k3_q * dt, p_curr + k3_p * dt)

        q_next = q_curr + 1.0 / 6 * (k1_q + 2 * k2_q + 2 * k3_q + k4_q) * dt
        p_next = p_curr + 1.0 / 6 * (k1_p + 2 * k2_p + 2 * k3_p + k4_p) * dt
        return q_next, p_next
